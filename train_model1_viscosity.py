...
LOSS_PLOT_FILE = "loss-model-1.png"
...
class EarlyStopping:
    def __init__(self, patience=30, min_delta=1e-5, path="es_best_model1.pth"):
...
def plot_scatter(preds, trues, epoch, folder="train_monitor", for_surface_tension=False,
...
def train():
    ...
    early = EarlyStopping(patience=100, min_delta=1e-5, path=os.path.join(OUTPUT_FOLDER, "es_best_model1.pth"))
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch % 100 == 0 or epoch == EPOCHS:
            ...
            if len(val_preds_abs) > 0:
                save_parity_csv(vpred, vtrue, val_names, epoch=epoch, split="val", folder=OUTPUT_FOLDER)
        ...
        if epoch % 500 == 0 or epoch == EPOCHS:
            save_path = os.path.join(OUTPUT_FOLDER, f"{NAME}_prediction_epoch{epoch}-model1.pth")
    ...
    if len(all_preds) > 0:
        ...
        plot_scatter(all_preds, all_trues, epoch="BEST", folder=OUTPUT_FOLDER,
                     for_surface_tension=False, split="test", filename_suffix="-model1")
