...
LOSS_PLOT_FILE = "loss-model-2-xgb.png"
...
def plot_loss(train_losses, val_losses, filename):
    if len(train_losses) == 0 or len(val_losses) == 0:
        return
    plt.figure()
    plt.semilogy(train_losses, label="Train (rmse)")
    plt.semilogy(val_losses, label="Val (rmse)")
...
def plot_scatter(preds, trues, epoch, folder="train_monitor",
                 for_surface_tension=True, split="val", filename_suffix=""):
    os.makedirs(folder, exist_ok=True)
    ...
    for i in range(num_targets):
        ...
        if for_surface_tension:
            ...
            ax.set_xlabel(r'True $\sigma$ (mN/m)'); ax.set_ylabel(r'Pred $\sigma$ (mN/m)')
        else:
...
def train_xgb():
    ...
    if train_metrics and val_metrics:
        ...
        plot_loss(tr_curve, va_curve, filename=LOSS_PLOT_FILE)
